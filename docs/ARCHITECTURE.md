...
graph TD
    ...
    subgraph honesty ["Held to account by CI"]
        ...
        CLAIMS["scripts/verify_claims.py<br/>24 documented claims re-derived"]
...
| [`scripts/verify_claims.py`](../scripts/verify_claims.py) | Documentation rots silently — a refactor moves a constant and the prose keeps quoting the old figure. Twenty-four documented claims are re-derived on every push. |
